...
const Product = ({ set_progress }) => {
  ...
  const fetch_product = async () => {
    ...
    const api_url = `${process.env.REACT_APP_SERVER_SIDE_URL}/api/product/fetch_product/customer?page=1`;
  ...
  const fetch_more_product = async () => {
    ...
    }/api/product/fetch_product/customer?page=${current_page + 1}`;
  ...
  useEffect(() => {
    if (localStorage.getItem("customer_authentication_token")) {
      document.title =
        "Products ||  Quantania - An efficient computer programs and application source code provider";
      fetch_product();
    } else {
