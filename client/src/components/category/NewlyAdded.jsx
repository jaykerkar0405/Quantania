...
const NewlyAdded = ({ set_progress }) => {
  ...
  const fetch_category = async () => {
    ...
    const api_url = `${process.env.REACT_APP_SERVER_SIDE_URL}/api/category/newly_added?page=1`;
    ...
    set_progress(75);
    set_category(response.newly_added);
  ...
  const fetch_more_category = async () => {
    ...
    }/api/category/newly_added?page=${current_page + 1}`;
    const request = await fetch(api_url, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
    ...
    const response = await request.json();
    set_progress(75);
    set_category(category.concat(response.newly_added));
    set_total_result(response.total_result);
    set_result(response.result);
  ...
  useEffect(() => {
    if (localStorage.getItem("customer_authentication_token")) {
      document.title =
        "Newly Added || Categories ||  Quantania - An efficient computer programs and application source code provider";
      fetch_category();
    } else {
      ...
      {loading ? (
        ...
        <main class="main">
          <div class="page-header text-center">
            <div class="container">
              <h1 class="page-title">
                Freshly added collections<span>Explore now</span>
              </h1>
            </div>
          </div>
