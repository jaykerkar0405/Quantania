...
const OrderDetails = () => {
  ...
  const [product_details, set_product_details] = useState({
    _id: "",
    product_image: "/image_preview.jpg",
  });
  ...
  const fetch_product_details = async () => {
    const api_url = `${process.env.REACT_APP_SERVER_SIDE_URL}/api/product/product_details/administrator`;
    const request = await fetch(api_url, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  return (
    <>
      ...
      {order_details.length > 0 && product_details.length > 0 ? (
                                    ...
                                    <div className="flex-lg-grow-1 ms-3">
                                      ...
                                      >
                                        <a
                                          ...
                                          href={`#/admin_panel/product_details?product_id=${element._id}&redirect=order`}
