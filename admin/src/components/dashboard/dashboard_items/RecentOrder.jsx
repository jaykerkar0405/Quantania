...
const RecentOrder = () => {
  ...
  const [recent_order, set_recent_order] = useState();
  const [loading, set_loading] = useState(true);

  const fetch_recent_order = async () => {
    set_loading(true);
    const request = await fetch(
      `${process.env.REACT_APP_SERVER_SIDE_URL}/api/order/recent_order`,
      {
        method: "GET",
        headers: {
    ...
    set_recent_order(response.recent_order);
    if (response) {
      set_loading(false);
    }
  };
                      ...
                      return (
                        ...
                        >
                          <div className="order-left-side">
                            Order id:{" "}
                            <a
                              href={`#/admin_panel/order_details?order_id=${element._id}&redirect=dashboard`}
                            ...
                            Product:{" "}
                            <a
                              href={`#/admin_panel/product_details?product_id=${element.product}&redirect=dashboard`}
                              rel="noopener noreferrer"
                              style={{ textDecoration: "none", color: "black" }}
                            >
