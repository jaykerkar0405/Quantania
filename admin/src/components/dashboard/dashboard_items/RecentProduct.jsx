...
const RecentProduct = () => {
  ...
  const [recent_product, set_recent_product] = useState();
  const [loading, set_loading] = useState(true);

  const fetch_recent_product = async () => {
    set_loading(true);
    const request = await fetch(
      `${process.env.REACT_APP_SERVER_SIDE_URL}/api/product/recent_product`,
      {
        method: "GET",
        headers: {
    ...
    set_recent_product(response.recent_product);
    if (response) {
      set_loading(false);
    }
  };
                ...
                <ul className="list-group" style={{ marginTop: "1rem" }}>
                  {recent_product.length === 0 && (
                    ...
                    >
                      ...
                      <div>No records has been added yet.</div>
                    </div>
                  )}
                  {recent_product.length > 0 &&
                    recent_product.map((element) => {
                      return (
                        <li
                        ...
                        >
                          <div className="product-left-side">
                            Name:{" "}
                            <a
                              href={`#/admin_panel/product_details?product_id=${element._id}&redirect=dashboard`}
                              target="_blank"
                              rel="noopener noreferrer"
                              style={{ textDecoration: "none", color: "black" }}
                            >
                              <span style={{ fontWeight: "bold" }}>
                                {element.product_name.length < 10
                                  ? element.product_name
                                  : `${element.product_name.slice(0, 10)}...`}
                              </span>
                            </a>
                            <br />
                            Price: <span>Rs {element.product_price}</span>
                          </div>
                          <div className="product-right-side d-flex">
                            <div
                              style={{
                                marginTop: "1.5rem ",
                                marginRight: "1rem",
