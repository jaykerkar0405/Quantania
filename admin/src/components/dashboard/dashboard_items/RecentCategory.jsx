...
const RecentCategory = () => {
  ...
  const [recent_category, set_recent_category] = useState();
  const [loading, set_loading] = useState(true);

  const fetch_recent_category = async () => {
    set_loading(true);
    const request = await fetch(
      `${process.env.REACT_APP_SERVER_SIDE_URL}/api/category/recent_category`,
      {
        method: "GET",
        headers: {
    ...
    set_recent_category(response.recent_category);
    if (response) {
      set_loading(false);
    }
  };
                      ...
                      return (
                        ...
                        >
                          <div className="category-left-side">
                            Name:{" "}
                            <a
                              href={`#/admin_panel/category_details?category_id=${element._id}&redirect=dashboard`}
                              target="_blank"
                            ...
                            >
                              <span style={{ fontWeight: "bold" }}>
                                {element.category_name.length < 20
                                  ...
                                  : `${element.category_name.slice(0, 20)}...`}
                              </span>
                            </a>
                            <br />
                            Status:{" "}
                            <Chip
                              sx={{
                                ...
                                pr: "4px",
                                backgroundColor:
                                  element.category_status === "active"
                                    ? "success.main"
                                    : "error.main",
                                color: "#fff",
                              }}
                          ...
                          <div className="category-right-side">
                            <a
                              href={element.category_image.image_url}
                              target="_blank"
                              rel="noopener noreferrer"
                            >
                              <img
